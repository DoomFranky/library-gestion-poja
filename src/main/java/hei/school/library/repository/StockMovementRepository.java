...
public interface StockMovementRepository extends JpaRepository<StockMovement, String> {
  @Query(
      "SELECT SUM("
          + "CASE WHEN sm.movementTypeEnum= hei.school.library.entity.MovementTypeEnum.OUT THEN sm.quantity * -1 ELSE sm.quantity END"
          + ") ,b.title, bf.format FROM Book b "
          + "JOIN b.bookFormat bf "
          + "JOIN BookCopy bc ON bc.bookFormat = bf "
          + "JOIN StockMovement sm ON sm.bookCopy = bc "
          + "WHERE b.id = :bookId ORDER BY (b.title, bf.format)")
  ...
  @Query(
      "SELECT SUM("
          ...
          + "JOIN BookCopy bc ON bc.bookFormat = bf "
          + "JOIN StockMovement sm ON sm.bookCopy = bc "
          + "WHERE bc.id = :bookCopyId ORDER BY (b.title,bf.format)")
